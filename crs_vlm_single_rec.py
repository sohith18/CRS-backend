...
Phase 2 (Direct Selection):
  • VLM compares remaining images pairwise until 1 survives

Pair selection (both phases):
  • right_pool fixed, left_pool cycles through alternatives on ambiguity
  • Phase 1: right_pool = ranked cluster B, left_pool = ranked cluster A
  • Phase 2: right_pool = [remaining[pair_index+1]], left_pool = all others
...
class Qwen3VLAgent:
    ...
    def generate_cluster_question(
        ...
        prompt = (
            ...
            "Generate exactly ONE NEW short question (max 30 words) to ask the user "
            "to understand which layout they prefer.\n"
            "Focus only on spatial/positional differences.\n"
            "Do NOT mention 'Image 1' or 'Image 2' — ask about layout features only.\n"
            "Output ONLY the question. No explanations, no numbering."
        )
...
def print_report(
    ...
    if process_eliminated:
        ...
        print(f"\n🔴  Eliminated during phases ({len(process_eliminated)} layouts):")
        for l in all_layouts:
            if l["id"] in elim_set:
                print(f"   • {l['id']}  ({l['image_path']})")
    else:
        print("\n🔴  Process-eliminated: none.")

    if winner_id:
        print(f"\n🏆  Winner: {winner_id}")

    print(f"\n[Summary] {len(process_eliminated)} / {len(all_layouts)} layouts not recommended.")
    print("=" * 55)


# ══════════════════════════════════════════════════════════════════════════════
# 7. UNIFIED PAIR SELECTOR  (same logic for Phase 1 and Phase 2)
# ══════════════════════════════════════════════════════════════════════════════
class PairSelector:
    """
    Unified anchor-based pair selector used identically in both phases.

    Concept:
      - right_pool: list of candidates for the "right" (anchor) slot
      - left_pool:  list of candidates for the "left" (rotating) slot

    Phase 1:  right_pool = ranked cluster B members (centroid-closest first)
              left_pool  = ranked cluster A members (centroid-closest first)

    Phase 2:  right_pool = [remaining[pair_index+1]]  (one fixed anchor)
              left_pool  = all remaining except right anchor

    On 'both':  anchor_step advances → next left_pool member paired with same right
    After left_pool exhausted: pair_index advances → next right_pool member, reset left
    """
    ...
    def on_clear_answer(self):
        """Reset anchor_step after a successful elimination."""
        self.anchor_step = 0

    def clamp(self, max_pair_index: int):
        self.pair_index  = min(self.pair_index, max_pair_index)
        self.anchor_step = 0
...
def main():
    ...
    while store.count > DIRECT_THRESHOLD:
        ...
        # Phase 1 pools: centroid-ranked members of each cluster
        # left_pool  = cluster A (all members, closest-first)
        # right_pool = cluster B (all members, closest-first)
        left_pool  = store.get_ranked_members(clusters[cid_a], cid_a, km)
        right_pool = store.get_ranked_members(clusters[cid_b], cid_b, km)

        # Reset selector at start of each new clustering round
        selector.reset()
        ...
        # ── inner loop: ask questions until cluster is eliminated ──────────
        eliminated = False
        while not eliminated:
            ...
            if is_unsure:
                ...
                print(f"[System] Rotating pair ({move}): "
                      f"next → {selector.get_pair(left_pool, right_pool)[0]['id']} "
                      f"vs {selector.get_pair(left_pool, right_pool)[1]['id']}")
                continue
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # PHASE 2 — Direct pairwise selection (same anchor logic)
    # ══════════════════════════════════════════════════════════════════════════
    print("\n" + "=" * 55)
    print(f" PHASE 2  —  Direct Selection  ({store.count} layouts left)")
    print("=" * 55)

    # Fresh selector for Phase 2
    selector.reset()
    ...
    while store.count > 1:
        ...
        # Phase 2 pools:
        # right_pool = [remaining[pair_index+1]]  — single fixed right anchor
        # left_pool  = all remaining except right anchor
        r_idx      = (selector.pair_index % (n - 1)) + 1
        right_pool = [remaining[r_idx]]
        left_pool  = [l for i, l in enumerate(remaining) if i != r_idx]

        rep_a, rep_b = selector.get_pair(left_pool, right_pool)
        img_a = rep_a["image_path"]
        img_b = rep_b["image_path"]

        print(f"\n[Phase 2] Comparing [{rep_a['id']}] vs [{rep_b['id']}]")

        if user_ctx.likes or user_ctx.dislikes:
            print(f"\n[Context] {user_ctx.as_prompt_block()}")
        ...
        if is_unsure:
            user_ctx.add_unsure(question[:60] + ("..." if len(question) > 60 else ""))
            print("[System] Got it — trying a different pair.")
            move = selector.advance_on_ambiguity(left_pool, right_pool)
            selector.clamp(max(store.count - 2, 0))
            print(f"[System] Rotating pair ({move})")
            continue
        ...
        eliminate_layout = rep_b if preferred == "1" else rep_a
        keep_layout      = rep_a if preferred == "1" else rep_b
        print(f"[System] Keeping: {keep_layout['id']} | Eliminating: {eliminate_layout['id']}")
        ...
        store.eliminate([eliminate_layout["id"]])
        selector.clamp(max(store.count - 2, 0))
    ...
    if store.active_layouts:
        ...
        print(f"    Image Path  : {winner['image_path']}")
    else:
        print("⚠  No layouts remaining.")

    surviving_ids      = {l["id"] for l in store.active_layouts}
    process_eliminated = [lid for lid in initial_active_ids if lid not in surviving_ids]
