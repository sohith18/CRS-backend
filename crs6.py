...
class QwenAgent:
    ...
    # ------------------------------------------------------------------
    def generate_question(self, axis_idx, vector_store, asked_questions, max_retries=6):
        """
        asked_questions: a collections.deque(maxlen=5) of the last 5 questions shown.
        No FAISS, no embeddings — just pass the 5 strings directly into the prompt
        and let the LLM avoid them.
        """
        high_desc, low_desc = vector_store.get_axis_contrast_pair(axis_idx)

        # Build the avoid block from the plain deque
        avoid_block = ""
        if asked_questions:
            avoid_block = (
                "These questions were already asked — write something DIFFERENT from all of them:\n"
        ...
        user_prompt = (
            f"{avoid_block}"
            f"Room contains: {FURNITURE_LABEL}.\n\n"
            f"Layout A:\n{high_desc}\n\n"
            f"Layout B:\n{low_desc}\n\n"
            "Write one simple question:"
        )

        _JARGON = re.compile(
            r"\b(perimeter|quadrant|density|zone|anchor|anchored|asymmetri|symmetri"
            r"|diagonal|functional|cluster|maximiz|minimiz|concentrat|geometric"
            r"|architectural|axis|axes|studio.functional)\b",
            re.IGNORECASE,
        )
        ...
        for attempt in range(max_retries):
            temp    = max(0.50 - 0.05 * attempt, 0.15)
            ...
            ok, reason = _is_ok(q)
            if not ok:
                print(f"    → INVALID   | {reason} | attempt={attempt+1} | {q[:80]}")
                continue

            # Accept on first valid question — LLM already knows to avoid the last 5
            print(f"    → ACCEPTED  | attempt={attempt+1} | {q}")
            asked_questions.append(q)
            return q

            best = q  # unreachable but kept for clarity

        # Fallback: best invalid candidate or hardcoded
        if best:
            print(f"    → FORCED    | {best}")
            asked_questions.append(best)
            return best
...
def main():
    ...
    # Plain rolling window of last 5 questions — no FAISS, no embeddings
    asked_questions = deque(maxlen=5)
    ...
    while turns_consumed < MAX_TURNS:
        ...
        question = llm.generate_question(axis_idx, vector_store, asked_questions)
        ...
        if not user_input:
            ...
            # Remove the question we just added so it doesn't count as asked
            if asked_questions:
                asked_questions.pop()
            continue
